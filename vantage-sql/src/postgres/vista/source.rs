    ...
    E: Entity<AnyPostgresType> + 'static,
{
    ...
    /// native `json`/`jsonb` column decodes to a map/array). Writes re-serialize
    /// the whole collection to JSON and `UPDATE` the column.
    fn get_contained_ref(&self, relation: &str, row: &Record<CborValue>) -> Result<Vista> {
        ...
        let columns: Vec<VistaColumn> =
            crate::postgres::vista::factory::metadata_from_table(&contained_table)
                .columns
                .into_values()
                .collect();
        ...
        let writeback: ContainedWriteback = Arc::new(move |collection: CborValue| {
            ...
            Box::pin(async move {
                ...
                let mut patch: Record<AnyPostgresType> = Record::new();
                patch.insert(host_column, AnyPostgresType::untyped(CborValue::Text(json)));
        ...
        let ref_resolver: ContainedRefResolver =
            Arc::new(move |relation: &str, child_row: &Record<CborValue>| {
                ...
                crate::postgres::vista::factory::PostgresVistaFactory::new(factory_db.clone())
                    .from_table(target)
            });
