...
async fn setup_clients_orders(suffix: &str) -> (MysqlDB, String, String) {
    let db = MysqlDB::connect(MYSQL_URL).await.unwrap();
    ...
    sqlx::query(&format!(
        "CREATE TABLE `{client_t}` (id VARCHAR(255) PRIMARY KEY, name TEXT NOT NULL)"
    ))
    .execute(db.pool())
    .await
    .unwrap();
    sqlx::query(&format!(
        "CREATE TABLE `{orders_t}` (id VARCHAR(255) PRIMARY KEY, client_id VARCHAR(255) NOT NULL, total BIGINT NOT NULL)"
    ))
    .execute(db.pool())
    .await
    .unwrap();
...
async fn vista_yaml_references_resolve_via_resolver() -> TestResult {
    ...
    use std::sync::Arc;
    use vantage_sql::mysql::vista::{MysqlSpecResolver, MysqlVistaSpec};
...
"#
    ...
    let client_spec: MysqlVistaSpec = serde_yaml_ng::from_str(&client_yaml)?;
    let orders_spec: MysqlVistaSpec = serde_yaml_ng::from_str(&orders_yaml)?;

    let map: IndexMap<String, MysqlVistaSpec> =
        [("orders".to_string(), orders_spec)].into_iter().collect();
    let map = Arc::new(map);
    let resolver: MysqlSpecResolver = Arc::new(move |name: &str| map.get(name).cloned());
...
"#;
    ...
    let client_spec: MysqlVistaSpec = serde_yaml_ng::from_str(&client_yaml)?;
    let vip_spec: MysqlVistaSpec = serde_yaml_ng::from_str(vip_yaml)?;
...
"#;
    ...
    let orders_spec: MysqlVistaSpec = serde_yaml_ng::from_str(&orders_yaml)?;
    let totals_spec: MysqlVistaSpec = serde_yaml_ng::from_str(totals_yaml)?;
    ...
    // MySQL `SUM(bigint)` returns DECIMAL, which surfaces as a CBOR decimal
