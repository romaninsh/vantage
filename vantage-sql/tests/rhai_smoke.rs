...
mod rhai_smoke {
    ...
    fn eval_rhai(code: &str) -> rhai::Dynamic {
        ...
        engine.eval(code).unwrap_or_else(|e| panic!("rhai eval failed: {e}"))
    }
    ...
    fn ident_creates_identifier() {
        ...
        let id = result.try_cast::<vantage_sql::rhai_engine::RhaiIdent>()
    ...
    fn expr_as_alias() {
        ...
        let ex = result.try_cast::<vantage_sql::rhai_engine::RhaiExpr<AnySqliteType>>()
    ...
    fn coalesce_with_mul() {
        ...
        let ex = result.try_cast::<vantage_sql::rhai_engine::RhaiExpr<AnySqliteType>>()
            .expect("should be RhaiExpr");
        assert_eq!(ex.0.preview(), r#"COALESCE(("ol"."quantity" * "ol"."price"), 0)"#);
    }

    // ── Checkpoint 3: Select builder tests ────────────────────────

    type TestSelect = vantage_sql::rhai_engine::RhaiSelect<
        AnySqliteType, SqliteSelect, SqliteSelectJoin, SqliteCondition,
    >;
    ...
    fn select_basic() {
        let result = eval_rhai(r#"
            select()
                ...
                .field("email")
        "#);
        let sel = result.try_cast::<TestSelect>().expect("should be RhaiSelect");
        assert_eq!(sel.inner.preview(), r#"SELECT "name", "email" FROM "users""#);
    }
    ...
    fn select_with_inner_join() {
        ...
        let sel = result.try_cast::<TestSelect>().expect("should be RhaiSelect");
        let sql = sel.inner.preview();
        assert!(sql.contains("INNER JOIN"), "should have INNER JOIN: {}", sql);
